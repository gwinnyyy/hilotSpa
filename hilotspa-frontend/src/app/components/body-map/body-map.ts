...
export class BodyMapComponent {
  ...
  regionNames: Record<string, string> = {
    'head': 'Head', 'neck': 'Neck', 'left-shoulder': 'Left Shoulder',
    'right-shoulder': 'Right Shoulder', 'upper-back': 'Upper Back',
    'lower-back': 'Lower Back', 'left-elbow': 'Left Elbow',
    'right-elbow': 'Right Elbow', 'left-wrist': 'Left Wrist',
    'right-wrist': 'Right Wrist', 'hip': 'Hip', 'left-knee': 'Left Knee',
    'right-knee': 'Right Knee', 'left-ankle': 'Left Ankle',
    'right-ankle': 'Right Ankle', 'chest': 'Chest', 'abdomen': 'Abdomen',
  ...
  onRegionClick(event: MouseEvent, regionId: string) {
    ...
    if (existing) {
      ...
      const newRegion: MarkedRegion = { anatomicalRegion: regionId, coordinateX: x, coordinateY: y, painScore: 5 };
  ...
  removeRegion(regionId: string) {
    ...
    if (this.selectedRegion?.anatomicalRegion === regionId) this.selectedRegion = null;
    this.regionsChanged.emit(this.markedRegions);
  }
